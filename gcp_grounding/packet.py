...
The Cloud Armor match-expression translator
(:mod:`gcp_grounding.armor_expr`) shares this module's source-address
variable: ``ArmorVars.src`` defaults to ``packet_vars(z3).src``, the 32-bit
BitVec stably named :data:`SRC_NAME`. Sharing that one constant is what lets a
firewall ``source_ranges`` entry and an Armor ``inIpRange`` predicate be
reasoned about together (a ``/16`` provably implies the enclosing ``/8``).
Because that translator abstains on ``ValueError`` from a bad CIDR,
:class:`UnsupportedPacket` derives from :class:`ValueError`: an IPv6 or
malformed range is *unsupported*, never false, under either ``except`` arm.
"""
...
class UnsupportedPacket(ValueError):
    ...
    Derives from :class:`ValueError` so that a caller which documents CIDR
    parsing as raising ``ValueError`` (:mod:`gcp_grounding.armor_expr` catches
    exactly that around :func:`cidr_match`) still abstains rather than
    propagating an unexpected exception through the fail-open gate.
    """
...
# every domain reasoning about a source IP reasons about the *same* z3 constant.
# ArmorVars.src reuses these directly, so they are public.
SRC_BITS = 32
SRC_NAME = "packet.src"
...
def packet_vars(z3, src=None, *, tags=(), service_accounts=()) -> PacketVars:
    ...
    *src* lets a caller thread in an already-minted source-address bitvector
    to share it (rather than the default :data:`SRC_NAME` constant); every
    other variable is always fresh.
    """
    _require(z3)
    tags = tuple(tags)
...
def parse_cidr(text: str) -> tuple[int, int]:
    ...
    :class:`UnsupportedPacket` (a :class:`ValueError`) — IPv4 only.
    ...
    treats the range. A non-string operand is rejected with the same
    *unsupported* verdict, never a false one.
    """
    if not isinstance(text, str):
        raise UnsupportedPacket(f"CIDR must be a string, got {type(text).__name__}")
    try:
...
def cidr_match(z3, addr, cidr: str):
    ...
    — ``(addr & mask) == (net & mask)``, the same encoding a firewall rule
    uses, so two ranges over the shared :attr:`PacketVars.src` are directly
    comparable. A ``/0`` prefix is ``BoolVal(True)`` (every address). An IPv6
    or malformed CIDR raises :class:`UnsupportedPacket`, itself a
    :class:`ValueError`."""
    _require(z3)
