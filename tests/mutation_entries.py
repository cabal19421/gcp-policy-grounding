...
``line_hint`` is RE-STAMPED. It was the design's number from the day of
measurement, and audit row R40 measured every entry's hint sitting OUTSIDE the
scope its anchor resolves in -- up to a thousand lines off, printed on every run
as the gate's own ``DRIFT`` line and pointing a reader at unrelated code. Each is
now the line ``mutation_contract.mutate`` resolves that entry's anchor at, which
is the number that same ``DRIFT`` line prints, so re-stamping is reading the
gate's output back. It stays ADVISORY and is still NEVER asserted: a later line
shift prints again rather than failing, which is the point of anchoring on
content. Every anchor is the content slice found in the owner's landed source,
widened to the smallest slice unique inside its ``enclosing`` scope -- the anchor
is what executes, and it never moved.
...
ENTRIES: tuple[Mutation, ...] = (
    ...
    Mutation(
        ...
        before="frozen=True", after="frozen=False", line_hint=100,
    ...
    Mutation(
        ...
        before='"icmp": 1', after='"icmp": 2', line_hint=292,
    ...
    Mutation(
        ...
        before='"udp": 17', after='"udp": 18', line_hint=292,
    ...
    Mutation(
        ...
        before='"ipv6-icmp": 58', after='"ipv6-icmp": 59', line_hint=293,
    ...
    Mutation(
        ...
        after='protocol is None and protocol == "all"', line_hint=411,
    ...
    Mutation(
        ...
        after="high - low + 1 >= MAX_PORT_SPAN", line_hint=442,
    ...
    Mutation(
        ...
        before="wide = True", after="wide = False", line_hint=445,
    ...
    Mutation(
        ...
        before="0 <= low", after="0 < low", line_hint=463,
    ...
    Mutation(
        ...
        before="high <= 65535", after="high <= 65536", line_hint=463,
    ...
    Mutation(
        ...
        before="exc_info=True", after="exc_info=False", line_hint=1603,
    ...
    Mutation(
        ...
        before="frozen=True", after="frozen=False", line_hint=161,
    ...
    Mutation(
        ...
        after="if isinstance(policy, Mapping):", line_hint=826,
    ...
    Mutation(
        ...
        after='bool(rule.get("enforce", True))', line_hint=854,
    ...
    Mutation(
        ...
        after="if isinstance(entry, str):", line_hint=868,
    ...
    Mutation(
        ...
        after="if positive_ok is None and negative_ok is None:", line_hint=979,
    ...
    Mutation(
        ...
        after='("; " - "; ".join(observed_empty))', line_hint=428,
...
#: owns were the first LIVE ones, and since audit row R43 corrected
#: `CONTRACT_CONTROL_SPAWNS` to the controls the contract really spends, NONE is
#: `pending` any more: the frozen gate EXECUTES every one of them.
REMOVALS: tuple[SeededRemoval, ...] = (
    ...
    SeededRemoval(
        ...
        #
        # LIVE since audit row R43. It was `pending`, and NOT because it does not
        # kill: MEASURED both ways in this checkout, both nodes report FAILED
        # under the mutant and PASSED on clean source. What held it was the SPAWN
        # CEILING alone -- ESC-GX-NETWORK-REMOVAL-CEILING carried the arithmetic,
        # and CONTRACT_CONTROL_SPAWNS is now the 21 controls it really has, so the
        # slot this entry was already counted for is spent on the child it needs.
        ...
        owner="gx-agentic-network-repin", pending=False, spelling=RE_MEASURED),
    SeededRemoval(
    ...
    SeededRemoval(
        ...
        must_fail=(_AV + "test_the_pair_check_decides_a_removal_against_a_"
                   ...
                   _AV + "test_a_widening_against_a_narrow_previous_policy_is_"
                         "decided"),
        owner="gx-agentic-vpcsc-repin", pending=False,
        spelling=AFTER_COLLECTION),
    SeededRemoval(
        ...
        owner="gx-agentic-vpcsc-repin", pending=False, spelling=IMPORT_TIME),
    SeededRemoval(
    ...
    SeededRemoval(
        ...
        #
        # LIVE since audit row R43. It was `pending`, and NOT because it does not
        # kill: `contract_spawn_ceiling()` is `4*len(register()) +
        # len(removal_register()) + CONTRACT_CONTROL_SPAWNS`, and a full run
        # measured the marked total at exactly the ceiling, so flipping an
        # ALREADY-COUNTED one from pending to live was +1 spawn and +0 slots and
        # overflowed by exactly one whatever else the diff did -- what
        # ESC-GX-ABSTAIN-REMOVAL-CEILING carried the arithmetic for. The five-slot
        # under-count in CONTRACT_CONTROL_SPAWNS was the cause, and correcting it
        # to 21 pays for this child and the four others that were parked with it.
...
DENY_ENTRIES: tuple[Mutation, ...] = (
    ...
    Mutation(
        ...
        before='        return _Tri("undecided", f"group membership of {member_v1!r} in {spelling!r} is not captured in any snapshot category (ESC-DENY-GROUP-MEMBERSHIP)")',
        after='        return _Tri("yes", f"group membership of {member_v1!r} in {spelling!r} is not captured in any snapshot category (ESC-DENY-GROUP-MEMBERSHIP)")',
        line_hint=208,
